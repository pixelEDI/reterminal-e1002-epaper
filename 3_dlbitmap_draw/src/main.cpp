...
const char *ssid = "bytebunker";
const char *password = "binarybuffer";
// const char *BMP_URL = "https://raw.githubusercontent.com/pixelEDI/"
//                       "reterminal-e1002-epaper/refs/heads/main/xkcd_bitmap.bmp";

const char *BMP_URL = "https://nodered.ipv64.net/xkcd.bmp";
...
void downloadBmpToSD() {
  Serial1.println("Verbinde mit WLAN...");
  ...
  HTTPClient http;
  http.setTimeout(20000); // 20 Sekunden wg download
  http.begin(BMP_URL);
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (!f) {
      Serial1.println("Fehler beim Öffnen von test.bmp zum Schreiben!");
      http.end();
      return;
    }
    ...
    uint8_t buf[2048];
    int len;
    Serial1.println("Lade und speichere BMP...");
    while ((len = stream->readBytes(buf, sizeof(buf))) > 0) {
      f.write(buf, len);
    }
    f.close();
    Serial1.println("Download abgeschlossen.");
  } else {
    Serial1.printf("HTTP Fehler: %d\n", httpCode);
  }
  http.end();
}
